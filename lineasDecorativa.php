...
$sql = "SELECT a.producto_slug, a.nombre, a.ruta_thumb, a.ruta_original, p.nombre_display, p.orden_listado, p.categoria
        FROM archivos a
        INNER JOIN (
...
// Lista de categorías presentes entre los productos ya cargados, en el orden
// preferido de negocio (si alguna categoría nueva aparece y no está en esta
// lista, igual se muestra, simplemente al final).
$ordenPreferido = ["Imprimante", "Sellador", "Pasta mural", "Temple", "Látex acabado", "Esmalte acabado"];
$categoriasEncontradas = [];
foreach ($productos as $p) {
    $cat = trim($p["categoria"] ?? "");
    if ($cat !== "" && !isset($categoriasEncontradas[$cat])) {
        $categoriasEncontradas[$cat] = slugCategoria($cat);
    }
}
...
<head>
    ...
    <title>Línea Decorativa - Jhomeron</title>
...
<body>
    ...
    <div class="arriba">
        <div class="arb">
            ...
            <div class="arb2">
                <h2>LÍNEA DECORATIVA</h2>

                <?php if (count($categoriasEncontradas) > 0): ?>
                ...
                <div class="mobile-filter-btn">
                    ...
                    <div id="mobileFilterMenu" class="mobile-filter-menu">
                        ...
                        <?php foreach ($categoriasEncontradas as $nombreCat => $slugCat): ?>
                            <div class="filter-item" data-filter="<?php echo htmlspecialchars($slugCat); ?>"><?php echo htmlspecialchars($nombreCat); ?></div>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <div class="arbtn">
                    <?php foreach ($categoriasEncontradas as $nombreCat => $slugCat): ?>
                        <button class="btn1" data-filter="<?php echo htmlspecialchars($slugCat); ?>"><?php echo htmlspecialchars($nombreCat); ?></button>
                    <?php endforeach; ?>
                </div>
                <?php endif; ?>
            </div>
    ...
    <script>
        ...
        // Filtro por categoría (Imprimante, Sellador, Pasta mural, etc.)
        document.addEventListener("DOMContentLoaded", function () {
            const cardsContainer = document.querySelector(".cards-pintura.desktop-version");
            const allCards = document.querySelectorAll(".product-card");
            const botonesDesktop = document.querySelectorAll(".arbtn .btn1");
            ...
            function resetearBotones() {
                botonesDesktop.forEach(btn => {
                    btn.style.backgroundColor = "#0d3393";
            ...
            filterItems.forEach(item => {
                item.addEventListener("click", function () {
                    ...
                    if (categoria === "all") {
                        ...
                        categoriaActiva = categoria;
                        const btnCorrespondiente = Array.from(botonesDesktop).find(b => b.getAttribute("data-filter") === categoria);
                        activarBoton(btnCorrespondiente);
                        filtrarPorCategoria(categoria);
                    }
                });
            });

            // Al cargar, se muestran todos los productos sin ningún botón activo
            mostrarTodos();
        });
