<?php
// lineaTrafico.php
// Página de línea de señalización (tráfico), conectada a la base de datos.
// Las tarjetas de producto se generan automáticamente desde la BD:
// por cada "producto_slug" distinto con linea='trafico', se muestra 1 tarjeta,
...
// Traemos 1 imagen representativa por cada producto de la línea de señalización (tráfico)
$sql = "SELECT a.producto_slug, a.nombre, a.ruta_thumb, a.ruta_original, p.nombre_display, p.orden_listado, p.categoria, p.grupo_filtro
        ...
        INNER JOIN (
            ...
            FROM archivos
            WHERE linea = 'trafico' AND tipo = 'imagen' AND producto_slug IS NOT NULL AND producto_slug != ''
...
<head>
    ...
    <title>Línea Señalización - Jhomeron</title>
            ...
            <div class="cards-row">
                <?php if (count($productos) === 0): ?>
                    <p style="font-family:'Outfit', sans-serif; padding: 20px;">
                        Aún no hay productos con línea "trafico" y un producto_slug asignado.
